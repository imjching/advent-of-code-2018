...
int main() {
  ...
  // We want highest overlaps among the guards
  int max_minute, minute_id, guard;
  max_minute = minute_id = guard = -INF;
  trav(a, schedule) {
    rep(i, 0, 60) {
      if (a.second[i] > max_minute) {
        max_minute = a.second[i];
        minute_id = i;
        guard = a.first;
      }
    }
  ...
  cout << "Guard #" << guard << " spent minute " << minute_id << " asleep." << endl;
